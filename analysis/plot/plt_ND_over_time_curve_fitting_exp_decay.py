...
def plt_ND_over_time_curve_fitting_exp_decay(df, colors, x_pos, option):
    ...
    plt.plot(x_extended, exp_growth(x_extended, *popt_exp), label=f"{option} (R²={r_squared_exp:.3f})",
             color=colors)
    ...
    # plt.show()
    plt.axhline(avg_pnd, x_0, 4, color=colors, label=f'avg PND last minute: {avg_pnd:.3f}', linestyle='dashed')
    ...
    # plt.text(x_pos, 0.15, f'shifted x axis: $y = {a_exp + c_exp:.3f}(1 - e^{{-{b_exp:.3f}(x+{abs(x_0):.3f})}})$',
    #          transform=plt.gca().transAxes, fontsize=10, verticalalignment='center', color=colors[1])
...
plt_ND_over_time_curve_fitting_exp_decay(df_RGN_nomp, colors[0], 0, 'RGN-nomp')
plt_ND_over_time_curve_fitting_exp_decay(df_AIB_nomp, colors[1], 0, 'AIB-nomp')
