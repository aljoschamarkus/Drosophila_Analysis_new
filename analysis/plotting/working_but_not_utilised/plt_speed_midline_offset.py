...
for cond_idx, cond in enumerate(condition):
    for geno_idx, geno in enumerate(genotype):
        ...
        speed_plt = df.xs((cond, geno), level=['condition', 'genotype'])['speed_manual'].values
...
for cond_idx, cond in enumerate(condition):
    for geno_idx, geno in enumerate(genotype):
        df.dropna(inplace=True)
        speed_plt = df.xs((cond, geno), level=['condition', 'genotype'])['speed_processed'].values
        speed_plt = speed_plt[speed_plt != 0]
...
#     for geno_idx, geno in enumerate(genotype):
#         df.dropna(inplace=True)
#         midline_offset_plt = df.xs((cond, geno), level=['condition', 'genotype'])['midline_offset_signless'].values
